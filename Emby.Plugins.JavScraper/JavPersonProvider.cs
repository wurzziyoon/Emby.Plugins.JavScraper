...
namespace Emby.Plugins.JavScraper
{
    public class JavPersonProvider : IRemoteMetadataProvider<Person, PersonLookupInfo>, IHasOrder
    {
        ...
        public async Task<MetadataResult<Person>> GetMetadata(PersonLookupInfo info, CancellationToken cancellationToken)
        {
            ...
            if (index != null) {
                _logger.Info(_jsonSerializer.SerializeToString(index));
            ...
            //if (doc == null)
            //{
            //    _logger?.Info($"{nameof(GetMetadata)} name:{info.Name} url:{index.Url} GetHtmlDocumentAsync failed.");
            //    return metadataResult;
            //}
        ...
        public async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(PersonLookupInfo searchInfo, CancellationToken cancellationToken)
        {
            ...
            foreach (var search_name in names)
            {
                ...
                var url = $"/search?q={search_name}&lg={lg}&f=actor";

                var doc = await AbstractScraper.GetHtmlDocumentAsync(client, url, _logger);
                if (doc == null)
                    continue;

                var actorsDiv = doc.DocumentNode.SelectSingleNode("//div[@id='actors']");
                string actor = null;
                string img = null;
                ...
                if (actorsDiv != null)
                {
                    // 获取 strong 标签中的文本（演员名）
                    var strongNode = actorsDiv.SelectSingleNode(".//strong");
                    if (strongNode != null)
                    {
                        actor = strongNode.InnerText.Trim();
                    }

                    // 获取 img 标签的 src 属性值
                    var imgNode = actorsDiv.SelectSingleNode(".//img[@class='avatar']");
                    if (imgNode != null)
                    {
                        img = imgNode.GetAttributeValue("src", null);
                    }

                    var aNode = actorsDiv.SelectSingleNode(".//a");
                    if (aNode != null)
                    {
                        actorUrl = aNode.GetAttributeValue("href", null);
                    }
                }
